...
#ifdef _WIN32
        static char iml_detector_env[256];
        snprintf(iml_detector_env, sizeof(iml_detector_env), "IML_DETECTOR_ID=%s", detector_id);
        _putenv(iml_detector_env);
#else
...
#ifdef _WIN32
        char *ctx = NULL;
        for (char *token = strtok_s(paths_copy, ";", &ctx); token; token = strtok_s(NULL, ";", &ctx)) {
#else
        char *saveptr = NULL;
        for (char *token = strtok_r(paths_copy, ";", &saveptr); token; token = strtok_r(NULL, ";", &saveptr)) {
#endif
            if (token[0] != '\0') {
