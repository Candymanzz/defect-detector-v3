...
public final class LightServerV3Http {
    ...
    public static void requireLightCommandSuccess(String endpointId, String httpMethod, String path, HttpResponse<String> response)
        ...
        if (status / 100 != 2) {
            throw new LightingException(endpointId + " " + httpMethod + " " + path + " failed status=" + status
                    + " body=" + body);
        }
        if (body.isBlank()) {
            return;
        ...
        try {
            ...
            if (root.has("success") && !root.get("success").asBoolean(false)) {
                ...
                throw new LightingException(endpointId + " " + httpMethod + " " + path + " success=false: " + err);
            }
        } catch (com.fasterxml.jackson.core.JsonProcessingException e) {
            throw new LightingException(endpointId + " " + httpMethod + " " + path + " invalid JSON: " + e.getMessage(), e);
        }
    }
}
