...
<body>
  ...
  <section id="defectPanel" class="panel hidden">
    ...
    <div class="muted">Одна кнопка сохранит все показанные контуры как отдельные примеры нормы с их примерным положением. Изменятся только будущие проверки; текущий результат никуда повторно не передаётся.</div>
...
<script>
  ...
  async function acceptAll() {
    ...
    button.disabled = true;
    document.getElementById('acceptStatus').textContent = 'Сохранение всех дефектов...';
    try {
      ...
      document.getElementById('acceptStatus').textContent = `Сохранено дефектов: ${Number(payload.accepted_count)}.${preview} Запустите проверку повторно.`;
