...
**Важно:** эталоны (`references`) хранятся **в памяти процесса** и теряются при перезапуске. JSON в `data/` — на диске (это FP зоны но при перезапуске они перезапишутся).
